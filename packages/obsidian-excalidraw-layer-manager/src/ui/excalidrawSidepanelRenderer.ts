...
import type { SidepanelLifecycleCallbacks } from "../runtime/sidepanelLifecycleBinding.js"
...
class ExcalidrawSidepanelRenderer implements LayerManagerRenderer {
  ...
  constructor(
    ...
    private readonly lifecycle?: SidepanelLifecycleCallbacks,
  ) {
    this.#host = host
    ...
    this.#mountManager = new SidepanelMountManager({
      ...
      lifecycle: {
        requestRefresh: () => {
          ...
          if (this.lifecycle) this.lifecycle.requestRefresh()
          else this.requestRenderFromLatestModel()
        },
        requestDispose: () => {
          this.dispose()
          this.lifecycle?.requestDispose()
        },
        onFocus: (view) => {
          this.lifecycle?.onFocus?.(view)
          this.handleSidepanelFocus()
        },
        onWindowMigrated: (win) => {
          this.attachOwnerDocumentKeyCapture(win.document)
        },
      },
    })
...
export const createExcalidrawSidepanelRenderer = (
  host: ExcalidrawSidepanelHost,
  lifecycle?: SidepanelLifecycleCallbacks,
  ...
  return new ExcalidrawSidepanelRenderer(host, lifecycle)
}
