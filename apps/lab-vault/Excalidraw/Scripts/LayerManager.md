...
(() => {
  ...
  var patchElementProperties = (target, elementPatch) => {
    ...
    if (set.name !== void 0 && target.type === "frame") target.name = set.name;
  };
  ...
  var planElementRename = (context, elementId, normalizedName) => {
    ...
    return ok({
      elementPatches: [
        {
          id: element.id,
          set: element.type === "frame" ? { name: normalizedName } : { customData: withLmxElementLabel(element.customData, normalizedName) }
        }
      ]
    });
  };
...
${err2.message}`);
  ...
  var executeIntentRequest = async (ea2, planner, canExecute) => {
    let attempts = 0;
    while (attempts < 2) {
      attempts += 1;
      ...
      runMutation: fromPromise(
        async ({
          ...
          if (input.request.type === "APPLY_REQUEST") {
            const outcome2 = input.request.canExecute() ? await applyPatch(input.ea, input.request.patch) : { status: "capabilityMissing", reason: "Scene ownership changed before execution." };
            return {
              kind: "apply",
...
${lines.join("\n")}`);
  ...
  // src/runtime/sidepanelLifecycleBinding.ts
  var owners = /* @__PURE__ */ new WeakMap();
  var hasSidepanelLifecycleOwner = (tab) => {
    return owners.has(tab);
  };
  var bindTargetView = (ea2, view) => {
    try {
      ea2.setView?.call(ea2, view, false);
    } catch {
    }
    if (ea2.targetView !== view) {
      try {
        ea2.targetView = view;
      } catch {
      }
  ...
  var createRuntimeSidepanelLifecycleBinding = (input) => {
    let disposed = false;
    let boundTab = null;
    let restore = [];
    const release = () => {
    ...
    const sync = () => {
      ...
      let closed = false;
      const owns = () => !disposed && !closed && boundTab === tab && owners.get(tab) === binding;
      const install = (key, handler) => {
  ...
  var SidepanelMountManager = class {
    ...
    constructor(input) {
      ...
      this.#lifecycle = createRuntimeSidepanelLifecycleBinding({
        ...
        ...input.lifecycle?.onWindowMigrated ? { onWindowMigrated: input.lifecycle.onWindowMigrated } : {}
      });
    }
    get #snapshot() {
    ...
    ensureSidepanelTab() {
      let failureReason = this.#creationFailed ? "tabCreationFailed" : null;
      ...
      if (!tab && !this.#creationFailed && this.#host.createSidepanelTab && !this.#snapshot.context.pendingTabCreation) {
        let created;
        try {
  ...
  var createExcalidrawSidepanelRenderer = (host, lifecycle) => {
    if (!hasDom()) {
      return null;
    }
  ...
  var createLayerManagerRuntime = (ea2, providedRenderer) => {
    ...
    let runtime = null;
    const renderer = providedRenderer ?? createExcalidrawSidepanelRenderer(ea2, {
    ...
    const reconcileHostContext = (signal) => {
      ...
      const result = hostFocusReleased ? hostContextCoordinator.reconcile("initial") : signal === "leaf-change" ? hostContextCoordinator.handleWorkspaceLeafChange() : signal === "poll" ? hostContextCoordinator.handlePollingFallback() : hostContextCoordinator.reconcile(signal);
    ...
    const subscribeToSceneChanges = () => {
      ...
      try {
        const unsubscribeCandidate = onChange.call(api, (_elements, appState) => {
          if (disposed || subscriptionGeneration !== sceneSubscriptionGeneration || api !== subscribedSceneChangeApi || currentSceneBindingKey !== subscribedSceneBindingKey)
            return;
    ...
    renderLatestSnapshot = () => {
      ...
      reconcileHostContext("manual");
      const nextSnapshot = hostFocusReleased ? { ...snapshot, version: snapshot.version + 1, elements: [], selectedIds: /* @__PURE__ */ new Set() } : readSnapshot(ea2);
      reconcileHostContext("manual");
    ...
    const captureMutationAuthority = () => {
      const epoch = hostAuthorityEpoch;
      const targetView = ea2.targetView;
      return () => !disposed && !hostFocusReleased && epoch === hostAuthorityEpoch && targetView === ea2.targetView;
    };
